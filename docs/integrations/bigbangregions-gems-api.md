...
- **On Success:** `BigBang Regions` calls `GemsService.capture(reservationId)`.
...
- **On Failure (before resize):** `BigBang Regions` calls `GemsService.release(reservationId)`.
...
- **Retry with Same Idempotency Key:** `BigBang Regions` MUST retry the `capture()` or `reserve()` call using the exact same `operationId` and `idempotencyKey`.
- **Idempotency Guarantees:**
  ...
  - Retrying `capture()` is fully idempotent. If the reservation is already captured, it returns success without deducting the balance twice.
