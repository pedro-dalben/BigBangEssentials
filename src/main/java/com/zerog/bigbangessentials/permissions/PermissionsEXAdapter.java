...
import com.zerog.bigbangessentials.api.permissions.PermissionScanner;
...
public class PermissionsEXAdapter implements ExternalPermissionAdapter {
    ...
    private static final String PEX_PERMISSIONS_FILE = com.zerog.bigbangessentials.util.ResourceUtil.DATA_DIR + "pex-permissions.txt";
    ...
    private void exportPermissionsForPEX() {
        try {
            ...
            File dataDir = new File(com.zerog.bigbangessentials.util.ResourceUtil.DATA_DIR);
            if (!dataDir.exists()) {
                if (!dataDir.mkdirs()) {
