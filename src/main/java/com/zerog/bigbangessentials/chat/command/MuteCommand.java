...
public class MuteCommand {
    ...
    private static int executeMute(com.mojang.brigadier.context.CommandContext<CommandSourceStack> ctx, String reason) {
        ...
        if (!com.zerog.bigbangessentials.config.ConfigManager.getInstance().isCommandEnabled("mute")) {
            source.sendFailure(MessageUtil.error("commands.bigbangessentials.mute.disabled"));
            return 0;
        }
        ...
        if (com.zerog.bigbangessentials.api.permissions.PermissionAPI.hasPermission(targetPlayer.getUUID(), "bigbangessentials.chat.mute.exempt")) {
            source.sendFailure(MessageUtil.error("commands.bigbangessentials.mute.exempt", targetName));
            return 0;
        }
        
        com.zerog.bigbangessentials.chat.MuteManager.mute(sender, targetName);
        // Notify Discord integrations
        try {
            com.zerog.bigbangessentials.integrations.ChatIntegrationManager.broadcastMuteEvent(targetPlayer, reason.isEmpty() ? "No reason given" : reason, true);
