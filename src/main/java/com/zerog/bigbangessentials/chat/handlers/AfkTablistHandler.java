    ...
    public static void updatePlayerTablistName(ServerPlayer player) {
        try {
            ...
            if (afkManager.isAfk(player.getUUID())) {
                ...
                com.zerog.bigbangessentials.util.DebugLogger.log(LOGGER, "Setting AFK tablist name for {}: {}", originalName, displayName);
            } else {
                // Player is not AFK - use original name
                displayName = originalName;
                com.zerog.bigbangessentials.util.DebugLogger.log(LOGGER, "Setting normal tablist name for {}: {}", originalName, displayName);
            }
