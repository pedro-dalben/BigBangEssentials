...
public class EnchantCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        if (!com.zerog.bigbangessentials.config.ConfigManager.getInstance().isCommandEnabled("enchant")) return;
            ...
            Commands.literal(commandName)
                .requires(cs -> cs.hasPermission(2) || // Allow ops
                    (cs.getEntity() instanceof ServerPlayer player && 
                     com.zerog.bigbangessentials.api.permissions.PermissionAPI.hasPermission(player.getUUID(), "bigbangessentials.item.enchant")))
                ...
                .then(Commands.argument("target", EntityArgument.player())
                    .requires(cs -> cs.hasPermission(2) || 
                        (cs.getEntity() instanceof ServerPlayer player && 
                         com.zerog.bigbangessentials.api.permissions.PermissionAPI.hasPermission(player.getUUID(), "bigbangessentials.item.enchant.others")))
                    .then(Commands.argument("enchantment", ResourceLocationArgument.id())
                        .suggests((ctx, builder) -> {
                            return net.minecraft.commands.SharedSuggestionProvider.suggestResource(
                                ctx.getSource().getServer().registryAccess()
    ...
    private static int executeEnchant(com.mojang.brigadier.context.CommandContext<CommandSourceStack> ctx, EnchantMode mode) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        ...
        boolean allowUnsafeEnchants = com.zerog.bigbangessentials.config.ConfigManager.isUnsafeEnchantsAllowed() ||
            com.zerog.bigbangessentials.api.permissions.PermissionAPI.hasPermission(executor.getUUID(), "bigbangessentials.item.enchant.unsafe");
        ...
        boolean canEnchantAny = com.zerog.bigbangessentials.api.permissions.PermissionAPI.hasPermission(executor.getUUID(), "bigbangessentials.item.enchant.any");
