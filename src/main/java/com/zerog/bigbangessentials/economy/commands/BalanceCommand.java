...
public class BalanceCommand {
        ...
        dispatcher.register(
            net.minecraft.commands.Commands.literal("balance")
                ...
                .then(net.minecraft.commands.Commands.argument("player", StringArgumentType.word())
                    .requires(src -> src.hasPermission(2) || com.zerog.bigbangessentials.api.permissions.PermissionAPI.hasPermission(src.getPlayer() != null ? src.getPlayer().getUUID() : null, "bigbangessentials.economy.balance.others"))
    ...
    private static int execute(com.mojang.brigadier.context.CommandContext<CommandSourceStack> ctx) {
        ...
        if (!com.zerog.bigbangessentials.api.permissions.PermissionAPI.hasPermission(player.getUUID(), "bigbangessentials.economy.balance")) {
    ...
    private static int executeOther(com.mojang.brigadier.context.CommandContext<CommandSourceStack> ctx) {
        ...
        if (!com.zerog.bigbangessentials.api.permissions.PermissionAPI.hasPermission(sender.getUUID(), "bigbangessentials.economy.balance.others")) {
            ctx.getSource().sendFailure(MessageUtil.error("commands.bigbangessentials.no_permission"));
            return 0;
        }
        String playerName = StringArgumentType.getString(ctx, "player");
        java.util.Optional<UUID> uuidOpt = com.zerog.bigbangessentials.economy.EconomyPlayerUtil.getUUIDByName(ctx.getSource().getServer(), playerName);
        if (uuidOpt.isEmpty()) {
            ctx.getSource().sendFailure(MessageUtil.error("commands.bigbangessentials.balance.player_not_found"));
            return 0;
        }
