...
public class SpawnManager {
    ...
    private void loadConfig() {
        try {
            ...
            if (configManager != null) {
                JsonObject config = configManager.getConfig(com.zerog.bigbangessentials.config.ConfigManager.MAIN_CONFIG);
    ...
    public boolean setSpawn(ServerPlayer setter, TeleportLocation location) {
        ...
        if (com.zerog.bigbangessentials.config.ConfigManager.getInstance().isLogSpawnActionsEnabled()) {
    ...
    public void teleportToSpawn(ServerPlayer player) {
        ...
        int maxDistance = com.zerog.bigbangessentials.config.ConfigManager.getInstance().getMaxTeleportDistance();
        if (maxDistance > 0 && spawnLocation != null) {
            com.zerog.bigbangessentials.teleportation.TeleportLocation fromLoc = new com.zerog.bigbangessentials.teleportation.TeleportLocation(player);
            if (fromLoc.getWorldName().equals(spawnLocation.getWorldName())) {
                double dist = fromLoc.distanceTo(spawnLocation);
                if (dist > maxDistance) {
                    player.sendSystemMessage(com.zerog.bigbangessentials.util.MessageUtil.error("commands.bigbangessentials.teleport.spawn.distance_exceeded", maxDistance));
                    return;
                }
            }
        }
        ...
        com.zerog.bigbangessentials.teleportation.Misc.MiscTeleportManager.getInstance().saveBackLocation(player);
