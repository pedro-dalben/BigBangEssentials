...
public class KickCommand {
    ...
    private static int executeKick(CommandContext<CommandSourceStack> ctx, String playerName, String reason) {
        ...
        try {
            ...
            String kickMessageTemplate = com.zerog.bigbangessentials.config.ConfigManager.getKickMessage();
            ...
            if (com.zerog.bigbangessentials.config.ConfigManager.isBroadcastKicksEnabled()) {
                for (ServerPlayer player : server.getPlayerList().getPlayers()) {
                    player.sendSystemMessage(MessageUtil.info(broadcastMsg));
                }
            }
            // Notify staff if enabled (independent of broadcastKicks)
            if (com.zerog.bigbangessentials.config.ConfigManager.isNotifyStaffOnKickEnabled()) {
                broadcastToStaff(server, broadcastMsg);
            }

            if (com.zerog.bigbangessentials.config.ConfigManager.isLogKickActionsEnabled()) {
    ...
    private static int executeKickAll(CommandContext<CommandSourceStack> ctx, String reason) {
        ...
        try {
            ...
            String kickAllMessageTemplate = com.zerog.bigbangessentials.config.ConfigManager.getKickAllMessage();
