...
public class ListKitsCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        if (!com.zerog.bigbangessentials.config.ConfigManager.isKitSystemEnabled()) {
    ...
    private static int listKits(CommandContext<CommandSourceStack> context, int page) {
        ...
        try {
            ...
            if (entity instanceof ServerPlayer player) {
                int cost = (int) com.zerog.bigbangessentials.config.ConfigManager.getKitCommandCost("listkits");
                if (cost > 0 && com.zerog.bigbangessentials.economy.managers.EconomyManager.getInstance().isEnabled()) {
                    ...
                    if (bal.doubleValue() < cost) {
                        source.sendFailure(com.zerog.bigbangessentials.util.MessageUtil.error("commands.bigbangessentials.listkits.not_enough_money", cost));
                        return 0;
                    }
                    if (!eco.subtractBalance(player.getUUID(), java.math.BigDecimal.valueOf(cost))) {
                        source.sendFailure(com.zerog.bigbangessentials.util.MessageUtil.error("commands.bigbangessentials.listkits.charge_failed"));
                        return 0;
                    }
                }
            }
            ...
            boolean skipUsedOneTime = com.zerog.bigbangessentials.config.ConfigManager.isSkipUsedOneTimeKitsFromKitList();
