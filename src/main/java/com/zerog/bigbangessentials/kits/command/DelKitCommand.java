    ...
    private static int confirmDeleteKit(CommandContext<CommandSourceStack> context) {
        ...
        try {
            ...
            if (entity instanceof ServerPlayer player) {
                int cost = (int) com.zerog.bigbangessentials.config.ConfigManager.getKitCommandCost("delkit");
                if (cost > 0 && com.zerog.bigbangessentials.economy.managers.EconomyManager.getInstance().isEnabled()) {
                    ...
                    if (bal.doubleValue() < cost) {
                        source.sendFailure(com.zerog.bigbangessentials.util.MessageUtil.error("commands.bigbangessentials.delkit.not_enough_money", cost));
                        return 0;
                    }
                    if (!eco.subtractBalance(player.getUUID(), java.math.BigDecimal.valueOf(cost))) {
                        source.sendFailure(com.zerog.bigbangessentials.util.MessageUtil.error("commands.bigbangessentials.delkit.charge_failed"));
                        return 0;
                    }
                }
            }
