...
public final class AdminShopTransactionService {
    ...
    private CompletableFuture<Boolean> restoreItems(Prepared p, ItemOutcome items) {
        ...
        return onServer(p.player, () -> p.buy ? (!items.command && remove(p.player, p.stack, items.amount) == items.amount)
                : addItemsStrict(p.player, ItemLoreHelper.copyWithCount(p.stack, items.amount)) == items.amount);
    }
    ...
    private boolean compensateItem(ServerPlayer player, ItemStack stack, int quantity, boolean buy, boolean itemApplied, boolean commandAttempted, int removed) {
        ...
        ItemStack restored = ItemLoreHelper.copyWithCount(stack, removed);
    ...
    private static long count(ServerPlayer player, ItemStack wanted) {
        long count = 0; for (ItemStack stack : player.getInventory().items) if (ItemStack.isSameItemSameTags(stack, wanted)) count += stack.getCount(); return count;
    }
    private static boolean hasRoom(ServerPlayer player, ItemStack wanted) {
        int free = 0; for (ItemStack stack : player.getInventory().items) free += stack.isEmpty() ? wanted.getMaxStackSize() : ItemStack.isSameItemSameTags(stack, wanted) ? Math.max(0, stack.getMaxStackSize() - stack.getCount()) : 0; return free >= wanted.getCount();
    }
    ...
    private static int remove(ServerPlayer player, ItemStack wanted, int amount) {
        int removed = 0; for (ItemStack stack : player.getInventory().items) if (amount > 0 && ItemStack.isSameItemSameTags(stack, wanted)) { int n = Math.min(amount, stack.getCount()); stack.shrink(n); amount -= n; removed += n; } return removed;
    }
