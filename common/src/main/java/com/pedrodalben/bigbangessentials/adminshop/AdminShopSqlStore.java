...
public final class AdminShopSqlStore {
    ...
    List<String> reconcile() {
        ...
        try {
            ...
            findings.addAll(db.queryList("adminshop reconcile.pending", "SELECT tx_id,status FROM adminshop_transaction_audit WHERE status IN ('STARTED','RESERVED','MONEY_APPLIED','ITEM_APPLIED','RECONCILIATION_REQUIRED')", null, r -> "PENDENCIA " + r.getString(1) + ":" + r.getString(2)).join());
