...
import com.pedrodalben.bigbangessentials.moderation.BanManager;
        ...
        dispatcher.register(Commands.literal("ban")
            ...
            .then(Commands.argument("player", StringArgumentType.greedyString())
                ...
                .executes(ctx -> executeBan(ctx, StringArgumentType.getString(ctx, "player"),
                    com.pedrodalben.bigbangessentials.config.ConfigManager.getInstance().getDefaultBanReason()))
                .then(Commands.argument("reason", StringArgumentType.greedyString())
                    .executes(ctx -> executeBan(ctx, 
                        ctx.getInput().split(" ", 3)[1], // Get player name 
                        ctx.getInput().split(" ", 3).length > 2 ? ctx.getInput().split(" ", 3)[2] : com.pedrodalben.bigbangessentials.config.ConfigManager.getInstance().getDefaultBanReason()))))
        );
        ...
        dispatcher.register(Commands.literal("banip")
            ...
            .then(Commands.argument("ip", StringArgumentType.word())
                .executes(ctx -> executeBanIP(ctx, 
                    StringArgumentType.getString(ctx, "ip"), com.pedrodalben.bigbangessentials.config.ConfigManager.getInstance().getDefaultBanReason()))
                .then(Commands.argument("reason", StringArgumentType.greedyString())
                    .executes(ctx -> executeBanIP(ctx,
